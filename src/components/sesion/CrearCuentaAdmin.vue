  ...
  export default {
    ...
    methods: {
      ...
      async register() {
        ...
        try {
          const response = await axios.post("http://localhost:5000/administradores", this.formData);
      ...
      async checkAccountNumber(accountNumber) {
        try {
          ...
          const { data } = await axios.get(`http://localhost:5000/administradores?accountNumber=${accountNumber}`);
